...
【重要】需要OASIS模拟环境正在运行才能使用此功能！""",
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Action:
        # {"name": "...", "parameters": {...}}
        action_pattern = r'Action:\s*(\{.*?\})(?=\s*(?:Observation:|Thought:|Final Answer:|$))'
    ...
    def plan_outline(
        ...
        # 构建规划prompt
        system_prompt = """你是一个「模拟分析报告」的撰写专家。你的职责不是扮演预言家，也不是把模拟世界写成已经发生的现实历史。

【核心原则】
1. 这是一份基于模拟结果和检索证据的分析报告，不是“未来已经确定发生”的宣告。
2. 你只能基于输入的模拟需求、图谱事实、以及后续工具真实返回的内容进行总结。
3. 你绝对禁止发明不存在的规则系统、协议名称、证据包编号、哈希、时间戳、地图坐标、机构、设备、API、平台日志。
4. 如果证据不足，必须明确写“证据不足”或“模拟未能支持确定结论”，不能补小说。
...
【报告定位】
- ✅ 这是一份基于模拟和检索证据的分析报告
- ✅ 聚焦于支持结论的证据、约束条件、不确定性
- ✅ 允许总结Agent行为，但必须标明其来自模拟而不是现实已发生事实
- ❌ 不要把模拟内容写成现实世界已发生、已提交、已触发的事件
- ❌ 不要虚构规则、协议、编号、密码学事件、地图判定机制
- ❌ 不要泛泛而谈地编造舆情综述
...
请基于可见的模拟事实和图谱上下文，设计报告章节：
1. 哪些结论是被模拟和检索证据支持的？
2. 哪些结论只是路径推演，不能当作已发生事实？
3. 哪些关键不确定性会影响最终判断？
...
【再次提醒】报告章节数量：最少2个，最多5个，内容要精炼聚焦于核心预测发现。"""
    ...
    def _generate_section_react(
        ...
        system_prompt = f"""你是一个「模拟分析报告」的撰写专家，正在撰写报告的一个章节。
...
模拟世界提供的是“可分析的推演结果”，不是现实世界已发生事件的自动记录。
你必须把“模拟中的路径、行为、言论”和“现实中已发生的事实”严格区分开。

你的任务是：
- 总结在设定条件下，模拟支持哪些判断
- 分析各类Agent（人群）是如何反应和行动的
- 识别值得关注的趋势、风险、不确定性和证据缺口

❌ 不要把模拟写成现实中已经发生的事件
❌ 不要发明任何不在工具结果或输入中的规则、协议、编号、时间戳、哈希、坐标、设备、组织
✅ 要明确区分“模拟推演显示”与“现实已确认”
...
1. 【必须调用工具观察模拟世界】
   - 所有内容必须来自工具真实返回的模拟世界事件、Agent言行或图谱事实
   - 禁止使用你自己的知识来编写报告内容
   - 每个章节至少调用2次工具（最多4次）来观察模拟的世界，它代表了未来

2. 【禁止伪造工具结果】
   - 只有系统真正执行后的工具返回，才可以写入 Observation 或 Final Answer
   - 如果你想调用工具，必须严格使用 <tool_call> ... </tool_call> 标签
   - 禁止自己在回答里编造“工具返回了什么”
...
5. 【证据等级约束】
   - 只有以下内容可以作为报告证据：
     a) 工具结果中明确列出的“当前有效事实”或等价事实列表
     b) 工具结果中返回的原始引用、原话、采访记录
     c) 工具结果中明确说明的统计值、状态值、结论
   - “涉及实体”“节点列表”“实体清单”“用户名列表”“标签列表”只能视为索引，不是证据
   - 绝对禁止仅凭实体名称、协议名称、标签、哈希样式字符串、编号样式字符串去推断规则、机构、流程、阈值、历史事件
   - 如果某个术语只出现在实体列表里，但没有出现在事实列表或原文引用里，则禁止写入报告正文
...
2. Action: 必须严格使用以下唯一格式调用工具：
   <tool_call>
   {{"name": "工具名称", "parameters": {{"参数名": "参数值"}}}}
   </tool_call>
3. Observation: [只分析系统真实返回的工具结果]
4. 重复步骤1-3，直到收集到足够信息（最多5轮）
5. Final Answer: [基于真实检索结果撰写章节内容]
...
8. 【再次强调】不要根据实体列表推断事实；实体名不是证据，只有事实列表和原文引用才是证据"""
...
5. 如果你没有收到真实工具结果，不要输出 Final Answer
...
记住：报告内容必须来自真实检索结果，而不是你的知识，也不是你伪造的 Observation！"""
                    })
                    continue
...
【记住】报告内容必须100%来自真实工具结果，不能使用你自己的知识，也不能在 Observation 里伪造工具返回！"""
                    })
                else:
                    # 已有足够调用，可以生成最终答案
                    messages.append({
                        "role": "user", 
                    "content": "你已经获取了足够的模拟数据。请基于检索到的信息，输出 Final Answer: 并撰写章节内容。\n\n【重要】内容必须大量引用检索到的原文，使用 > 格式引用；严禁把实体列表、标签、协议名、编号名当成事实证据。"
                })
                continue
...
- 如果需要更多信息：继续调用工具检索
- 禁止根据“涉及实体/实体清单”推断事实；只能依据事实列表、原始引文、明确统计值写作

已调用工具 {tool_calls_count}/{self.MAX_TOOL_CALLS_PER_SECTION} 次
═══════════════════════════════════════════════════════════════"""
            })
        
        # 达到最大迭代次数时，如果真实工具调用不足，直接失败，避免无根据胡编
        if tool_calls_count < min_tool_calls:
            logger.error(f"章节 {section.title} 未完成最低检索要求，拒绝生成无依据内容")
            final_answer = "（本章节生成失败：未完成最低检索要求，系统拒绝在证据不足时编写报告内容。）"
            is_subsection = section_index >= 100
            if self.report_logger:
                self.report_logger.log_section_content(
                    section_title=section.title,
        ...
        # 达到最大迭代次数，但已有足够检索，允许收尾
        logger.warning(f"章节 {section.title} 达到最大迭代次数，进入受限收尾")
        messages.append({
            "role": "user",
            "content": "已达到工具调用限制。请只基于真实工具结果输出 Final Answer，严禁补写任何未检索到的规则、编号、时间戳、机构、设备或事件。再次提醒：实体清单不是证据，不能写入正文。"
        })
